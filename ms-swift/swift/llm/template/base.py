...
class Template(ProcessorMixin):
        ...
        for img_field in ['images', 'rejected_images']:
            ...
            if images:
                for i, image in enumerate(images):
                    ...
                    #print(f"load image cost time: {time.time() - start_time}")
            if inputs.objects:
    ...
    def _data_collator_mm_data(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        npr_image_tensor = [b['npr_image_tensor'] for b in batch if b.get('npr_image_tensor') is not None]
        if len(npr_image_tensor) > 0:
            res['npr_image_tensor'] = torch.concat(npr_image_tensor)
            
        npr_image_tensor = [b['npr_pixel_values'] for b in batch if b.get('npr_pixel_values') is not None]
        if len(npr_image_tensor) > 0:
            res['npr_pixel_values'] = torch.concat(npr_image_tensor)
        return res
