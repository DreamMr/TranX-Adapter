...
#     K.RandomGaussianBlur(kernel_size=(3, 3), sigma=(0.1, 3.0), p=1.),
#     K.RandomJPEG(jpeg_quality=(30, 100), p=1.)
# )
...
def sampling_resolution(min_size=128, max_size=896, step=128):
...
def data_augment(img, blur_prob, blur_sig, jpg_prob, jpg_qual, jpg_method):
    img = np.array(img)
...
class LlavaHfTemplate(Template):
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        if images:
            ...
            images = [Perturbations(image) for image in images]
            clip_images = [self.transform(image) for image in images]
            image_inputs = image_processor(clip_images, return_tensors='pt').to(self.model_info.torch_dtype)
            encoded['pixel_values'] = image_inputs['pixel_values']
            if self.processor.vision_feature_select_strategy == 'expert':
                npr_image_tensor = [self.npr_transform(image) for image in images]
                npr_image_tensor = torch.stack(npr_image_tensor,dim=0)
                encoded['npr_image_tensor'] = npr_image_tensor
            ...
            if version.parse(transformers.__version__) >= version.parse('4.47'):
                ...
                for i, idx in enumerate(idx_list):
                    ...
                    if self.processor.vision_feature_select_strategy == 'default':
                        num_image_tokens -= 1
                    elif self.processor.vision_feature_select_strategy == 'expert':
                        #num_image_tokens += 1
...
register_template(
    TemplateMeta(
        ...
        #MLLMTemplateType.llava1_6_mistral_hf,
        prefix=['<s>'],
        prompt=['USER: {{QUERY}} ASSISTANT:'],
        chat_sep=['</s>'],
        suffix=['</s>'],
