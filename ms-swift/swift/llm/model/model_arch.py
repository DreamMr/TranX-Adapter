...
class MultiModelKeys(ModelKeys):
    ...
    vision_tower_expert: Union[str, List[str]] = field(default_factory=list)
    aligner_expert: Union[str, List[str]] = field(default_factory=list)

    def __post_init__(self):
        for key in ['language_model', 'aligner', 'vision_tower', 'generator', 'vision_tower_expert', 'aligner_expert']:
            v = getattr(self, key)
            if isinstance(v, str):
                setattr(self, key, [v])
            if v is None:
...
if transformers_ge_4_52:
    register_model_arch(
        MultiModelKeys(
            ...
            mlp='language_model.model.layers.{}.mlp',
...
if transformers_ge_4_52:
    register_model_arch(
        MultiModelKeys(
            MLLMModelArch.llava_npr_hf,
            mlp='model.language_model.model.layers.{}.mlp',
            language_model='model.language_model',
            aligner='model.multi_modal_projector',
            vision_tower='model.vision_tower',
            vision_tower_expert='model.vision_tower_expert',
            aligner_expert='model.multi_modal_expert_projector',
    ...
    register_model_arch(
        MultiModelKeys(
            ...
            aligner='multi_modal_projector',
            vision_tower='vision_tower',
            vision_tower_expert='vision_tower_expert',
            aligner_expert='multi_modal_expert_projector',
        ))
