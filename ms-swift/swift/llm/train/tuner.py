...
def get_multimodal_target_regex(
    ...
    *,
    ...
    freeze_aligner_expert: bool = True,
    freeze_vit_expert: bool = True,
    ...
    if not freeze_aligner_expert:
        modules += model_arch.aligner_expert
    if not freeze_vit_expert:
        modules += model_arch.vision_tower_expert
    ...
    if 'all-linear' in target_modules:
        if model_meta.is_multimodal:
            return get_multimodal_target_regex(
                ...
                freeze_aligner_expert=args.freeze_aligner_expert,
                freeze_vit_expert=args.freeze_vit_expert,
...
def prepare_adapter(args: TrainArguments, model, *, template=None, train_dataset=None, task_type=None):
    ...
    if args.train_type in ('lora', 'longlora'):
        ...
        target_modules = [mlp_key.format(i) for i in range(len(model.language_model.model.layers))]
        adapter_config = AdapterConfig(
            dim=model.config.hidden_size if hasattr(model.config, "hidden_size") else model.config.text_config.hidden_size,
