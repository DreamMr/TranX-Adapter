...
class Seq2SeqTrainer(SwiftMixin, DataLoaderMixin, HfSeq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if hasattr(outputs, "cos_loss") and outputs.cos_loss is not None:
            if 'cos_loss' not in self._custom_metrics:
                self._custom_metrics['cos_loss'] = MeanMetric(nan_value=None)
            self._custom_metrics['cos_loss'].update(outputs.cos_loss.detach().item())

        if hasattr(outputs, "kl") and outputs.kl is not None:
            if 'kl' not in self._custom_metrics:
                self._custom_metrics['kl'] = MeanMetric(nan_value=None)
            self._custom_metrics['kl'].update(outputs.kl.detach().item())

        if hasattr(outputs, "ar_loss") and outputs.ar_loss is not None:
            if 'ar_loss' not in self._custom_metrics:
                self._custom_metrics['ar_loss'] = MeanMetric(nan_value=None)
            self._custom_metrics['ar_loss'].update(outputs.ar_loss.detach().item())

        if hasattr(outputs, "cos") and outputs.cos is not None:
            if 'cos' not in self._custom_metrics:
                self._custom_metrics['cos'] = MeanMetric(nan_value=None)
            self._custom_metrics['cos'].update(outputs.cos.detach().item())

        if hasattr(outputs, "orth_loss") and outputs.orth_loss is not None:
            if 'orth_loss' not in self._custom_metrics:
                self._custom_metrics['orth_loss'] = MeanMetric(nan_value=None)
            self._custom_metrics['orth_loss'].update(outputs.orth_loss.detach().item())
